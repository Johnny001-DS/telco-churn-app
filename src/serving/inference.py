...
try:
    ...
    # Fallbacks for local development (helps when running outside Docker)
    try:
        import glob

        # 1) Check local mlruns (generated after running training pipeline)
        local_model_paths = glob.glob("./mlruns/*/*/artifacts/model")

        # 2) Check bundled dev model under src/serving/model
        bundled_model_paths = glob.glob("src/serving/model/*/artifacts/model")

        candidate_paths = local_model_paths + bundled_model_paths
        if candidate_paths:
            latest_model = max(candidate_paths, key=os.path.getmtime)
            model = mlflow.pyfunc.load_model(latest_model)
            MODEL_DIR = latest_model
            print(f"✅ Fallback: Loaded model from {latest_model}")
        else:
            raise Exception("No model found in mlruns or src/serving/model")
...
try:
    # Prefer feature_columns.txt alongside the model; fallback to one level up (artifacts root)
