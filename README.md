...
### Docker image with a specific MLflow run
- The Dockerfile bundles a trained model from `mlruns/712268213209809681/ebb1203e5b044bd29bca61311a07d096/artifacts` by default.
- To bundle a different run, pass `MODEL_RUN_PATH` at build time:
  ```
  docker build -t telco-churn-app \
    --build-arg MODEL_RUN_PATH=mlruns/<exp_id>/<run_id>/artifacts .
  ```
- Run locally: `docker run -p 8000:8000 telco-churn-app`
- Test: `curl http://localhost:8000/` (health), `python scripts/test_fastapi.py` (predict), or open `http://localhost:8000/ui`.
